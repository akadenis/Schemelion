...
type
  ...
  TEvsMain = class(TForm)
    ...
    actColorRed: TAction;
    actColorYellow: TAction;
    actColorBlack: TAction;
    actColorGreen: TAction;
		actColorGray: TAction;
		actColorBlue: TAction;
		actElemInput: TAction;
		actElemOutput: TAction;
    act_2Or: TAction;
    ActSetGridSizeMinus: TAction;
    actSetGridSizePlus: TAction;
    actViewGrid : TAction;
    ...
    StaticText4 : TStaticText;
		tbColorBlue: TToolButton;
    tbColorYellow: TToolButton;
    tbColorGreen: TToolButton;
    tbColorGrey: TToolButton;
    tbColorBlack: TToolButton;
    tbGridDist2: TToolButton;
		tbtnElemInput: TToolButton;
		tbtnRectNode2: TToolButton;
    tbtn_2Or: TToolButton;
    tbtn_2And: TToolButton;
    ToolBar1: TToolBar;
    ToolBar2 : TToolBar;
    ...
    tbGridDist: TToolButton;
    ToolButton20: TToolButton;
    tbColorRed: TToolButton;
    ...
    procedure actHexagonNodeUpdate(Sender : TObject);
		procedure actElemInputExecute(Sender: TObject);
    procedure ActSetGridSizeMinusExecute(Sender: TObject);
    procedure actSetGridSizePlusExecute(Sender: TObject);
    ...
    procedure actSetGridSizeUpdate(Sender: TObject);
    ...
    procedure act_2AndExecute(Sender: TObject);
    procedure act_2OrExecute(Sender: TObject);
...
procedure TEvsMain.actElemInputExecute(Sender: TObject);
begin
  Test.CommandMode := cmInsertNode;
  Test.DefaultNodeClass := TAd_Node;
end;

procedure TEvsMain.ActSetGridSizeMinusExecute(Sender: TObject);
begin
  Test.GridSize := Test.GridSize - 1;
end;

procedure TEvsMain.actSetGridSizePlusExecute(Sender: TObject);
begin
  Test.GridSize := Test.GridSize + 1;
end;
...
procedure TEvsMain.actSetGridSizeUpdate(Sender: TObject);
begin
...
procedure TEvsMain.actColorExecute(Sender: TObject);
var
  I: Integer;
  bt : TAction;
  cl : TColor;
begin
  ...
  bt := TAction(Sender);
  try
  if(bt.Name = 'actColorRed') then
    cl := clRed
  else if(bt.Name = 'actColorYellow') then
    cl := clYellow
  else if(bt.Name = 'actColorGreen') then
    cl := clGreen
  else if(bt.Name = 'actColorBlue') then
    cl := clBlue
  else if(bt.Name = 'actColorGray') then
    cl := clGray
  else if(bt.Name = 'actColorBlack') then
    cl := clBlack;


  for I := 0 to Test.SelectedObjects.Count - 1 do
      with TEvsGraphObject(Test.SelectedObjects[I]) do
      begin
        BeginUpdate;
        try
          Pen.Color := cl;
...
procedure TEvsMain.act_2AndExecute(Sender: TObject);
begin
  Test.CommandMode := cmInsertNode;
  Test.DefaultNodeClass := TAd_Node;
  //Test.ElemLeftPins := 2;
  //Test.ElemRightPins := 1;
  //Test.ElemText := '&';
end;

procedure TEvsMain.act_2OrExecute(Sender: TObject);
begin
  Test.CommandMode := cmInsertNode;
  Test.DefaultNodeClass := TAd_Node;
end;
