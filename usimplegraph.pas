...
{$ENDIF}
  ...
  TEvsGraphNode = class(TEvsGraphObject)
  //DrawText replacement required.  Region alternative
  public
    ElemLeftPins, ElemRightPins: Integer;
    ElemText : String;
  ...
  TAd_Node = class(TEvsPolygonalNode)
  private
    pinLen, pinNum: Integer;
  protected
    //function QueryHitTest(const Pt : TPoint) : DWORD; override;
    function CreateRegion: HRGN;                                       override;
    procedure DrawBorder  (aCanvas: TCanvas);                          override;
...
{$REGION ' TAd_Node '}
function TAd_Node.CreateRegion : HRGN;
begin
  pinLen := PIN_LEN;
  pinNum := 2;// по умолчанию
  Result := inherited CreateRegion;

end;

procedure TAd_Node.DefineVertices(const ARect: TRect; var Points: TPoints);
begin
  SetLength(Points, 4);
  Points[0].X := ARect.Left+pinLen;
  Points[0].Y := ARect.Top;
  Points[1].X := ARect.Right-pinLen;
  Points[1].Y := ARect.Top;
  Points[2].X := ARect.Right-pinLen;
  Points[2].Y := ARect.Bottom;
  Points[3].X := ARect.Left+pinLen;
  Points[3].Y := ARect.Bottom;
end;

procedure TAd_Node.DrawBorder(aCanvas: TCanvas);
var
  pnts, pnts1 : TPoints;
	vPen :TPen;
	VBrush : TBrush;
  dh, h, i: Integer;
begin
  pnts := Copy(Vertices,0,Length(Vertices));
  vPen := aCanvas.Pen;
  VBrush := aCanvas.Brush;
  pnts1 := Copy(pnts, 0, Length(pnts));
  pnts1[0].X :=  pnts1[0].X - pinLen;
  pnts1[1].X :=  pnts1[1].X + pinLen;
  pnts1[2].X :=  pnts1[2].X - pinLen;
  pnts1[3].X :=  pnts1[3].X + pinLen;

  aCanvas.Polygon(pnts);

  // входные пины (слева): рассчитываем расстояния по оси Y в зависимости от их количества
  h := pnts[2].Y - pnts[0].Y; // высота собственно граф.элемента
  dh := Integer(h div (ElemLeftPins+1)); // расстояние между пинами

  for i := 0 to ElemLeftPins do
  begin
    aCanvas.Line(pnts[0].X, pnts[0].Y + dh, pnts[0].X - pinLen, pnts[0].Y+dh);
    aCanvas.Line(pnts[0].X, pnts[0].Y + dh*2, pnts[0].X - pinLen, pnts[0].Y+dh*2);
  end;

  // выходные пины
  dh := (h div 2);
  aCanvas.Line(pnts[1].X, pnts[1].Y + dh, pnts[1].X + pinLen, pnts[1].Y+dh);
...
begin
  ...
  TEvsSimpleGraph.Register(TAd_Node);
end;
...
begin
  ...
  TEvsSimpleGraph.Unregister(TAd_Node);
end;
