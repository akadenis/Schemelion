...
class function TEvsLinkProperties.Execute(Links: TEvsGraphObjectList): Boolean;
begin
  Result := False;
...
procedure TEvsLinkProperties.ApplyChanges;
var
  I: Integer;
begin
  S.BeginUpdate;
...
procedure TEvsLinkProperties.LinkLineColorClick(Sender: TObject);
begin
...
procedure TEvsLinkProperties.LinkStyleColorClick(Sender: TObject);
begin
...
procedure TEvsLinkProperties.btnChangeFontClick(Sender: TObject);
begin
  FontDialog.Execute;
end;

procedure TEvsLinkProperties.btnApplyClick(Sender: TObject);
begin
  ApplyChanges;
end;

procedure TEvsLinkProperties.FormCreate(Sender: TObject);
begin
  SetBounds(Screen.Width - Width - 30, 50, Width, Height);
end;

function TEvsLinkProperties.GetObjectOptions: TEvsGraphObjectOptions;
var
...
procedure TEvsLinkProperties.SetObjectOptions(Value: TEvsGraphObjectOptions);
var
  Option: TEvsGraphObjectOption;
begin
  for Option := Low(TEvsGraphObjectOption) to High(TEvsGraphObjectOption) do
...
function TEvsLinkProperties.GetLinkOptions: TEvsGraphLinkOptions;
var
...
procedure TEvsLinkProperties.SetLinkOptions(Value: TEvsGraphLinkOptions);
var
  Option: TEvsGraphLinkOption;
begin
  for Option := Low(TEvsGraphLinkOption) to High(TEvsGraphLinkOption) do
